...
- **Estado Actual**: **Fase 5 — SUNARP EN PROGRESO (2026‑09‑19, T5.1 Conoce Aquí + T5.2 Consulta de Propiedad + T5.3 SPRL + T5.4 Registry normalization DONE)**. Fase 4 — REM@JU COMPLETED (T4.1–T4.9: discovery, parser, normalization, dedup, linking + intake manual, research connector, manual action handling, tests, monitoring). Siguiente tarea **T5.5 (Owners)**. SUNARP no tiene superficie consultable sin identidad + CAPTCHA → postura `requires_auth` (conectores reales de postura, ver §3.4). Ver `PROJECT_STATUS.md` (estado vivo) y `PROJECT_EXECUTION_PLAN.md` (plan maestro). Fases 0–3 completadas (infra local, arquitectura, ingesta SQLite→PostgreSQL, conector OSM/Nominatim real, workers BullMQ, Research Engine T3.x).
...
│   ├── tests/                 # Suite de pruebas Vitest (187 tests pasando)
...
│   │   └── sunarp-normalize.test.ts # Tests de normalización registral SUNARP (T5.4)
│   └── fixtures/
...
│       │       └── sunarp-normalize.ts # Normalización registral SUNARP (clave canónica P-XXXXXXXX + captura) (T5.4)
│       ├── db/
...
└── docs/                      # ── DOCUMENTACIÓN COMPLETA ──
    ...
    ├── SUNARP.md              # Reporte de discovery SUNARP + Fase 5 (T5.1–T5.4)
...
```bash
cd server
npm.cmd test               # Ejecuta Vitest (187 tests automáticos)
