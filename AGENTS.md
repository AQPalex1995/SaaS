...
- **Estado Actual**: **Fase 4 EN CURSO (REM@JU, aprobada por el usuario)** — T4.1 a T4.7 DONE (discovery, parser, normalization, dedup, linking + intake manual, research connector, manual action handling); siguiente tarea **T4.8 tests**. Ver `PROJECT_STATUS.md` (estado vivo) y `PROJECT_EXECUTION_PLAN.md` (plan maestro). Fases 0–3 completadas (infra local, arquitectura, ingesta SQLite→PostgreSQL, conector OSM/Nominatim real, workers BullMQ, Research Engine T3.x).
...
│   ├── tests/                 # Suite de pruebas Vitest (146 tests pasando)
...
```bash
cd server
npm.cmd test               # Ejecuta Vitest (146 tests automáticos)
