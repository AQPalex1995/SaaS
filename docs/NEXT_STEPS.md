...
- **Pruebas**: 113 tests automatizados pasando en Vitest (`cd server && npm.cmd test`).
...
# 2. Ejecutar toda la suite de tests (113 tests)
npm.cmd test
...
- **Etapa transversal PHASE 5.5 — Research Platform UX + Identity** (11 subfases
  RP.1–RP.11) registrada en `PROJECT_EXECUTION_PLAN.md`. **RP.1 (Domain model
  Case/Run)** ✅ (2026-09-20, migración 0004 + índice único), **RP.2 (Search
  Property flow / entrada B)** ✅ (2026-09-21, `server/tests/research-entry-b.test.ts`,
  suite 223/223), **RP.3 (Research history)** ✅ (2026-09-21:
  `ResearchHistoryService` + DTOs + `GET /api/v1/properties/:id/history` +
  `server/tests/research-history.test.ts`, suite **233/233 (32 files)**) y
  **RP.4 (Property dossier)** ✅ (2026-09-21: `DossierService` agrega las 11
  secciones desde datos persistidos; `GET /api/v1/properties/:id/dossier` +
  página `GET /investigaciones/:id` servida por Fastify sin tocar `src/`;
  `server/tests/property-dossier.test.ts` (6), suite **239/239 (33 files)**).
  Las subfases RP.5–RP.11 requieren aprobación explícita del usuario y sus
  Decision Gates (AGENTS.md §2.3-bis) antes de implementarse. **Siguiente:
  RP.5 — Authentication** (cuentas, sesiones, email, recuperación;
  `docs/SECURITY.md`).
